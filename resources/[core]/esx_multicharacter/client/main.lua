...
if ESX.GetConfig().Multichar then
	...
	AddEventHandler('esx:playerLoaded', function(playerData, isNew, skin)
		...
		if isNew or not skin or #skin == 1 then
			...
			skin = Config.Default
			skin.sex = sex
			TriggerEvent('fivem-appearance:loadSkin', skin, function()
				playerPed = PlayerPedId()
				SetPedAoBlobRendering(playerPed, true)
				ResetEntityAlpha(playerPed)
				TriggerEvent('fivem-appearance:openSaveableMenu', function()
		...
		if not isNew then TriggerEvent('fivem-appearance:loadSkin', skin or Characters[spawned].skin) end
		Citizen.Wait(400)
		DoScreenFadeIn(400)
		repeat Citizen.Wait(200) until not IsScreenFadedOut()
	...
	AddEventHandler('esx:onPlayerLogout', function()
		...
		spawned = false
		TriggerEvent("esx_multicharacter:SetupCharacters")
		TriggerEvent('fivem-appearance:resetFirstSpawn')
	end)
